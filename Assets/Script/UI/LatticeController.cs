...
public class LatticeController : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public event ItemInBagController.ClickItem UseThisItemCallBack;
    [HideInInspector]
    ...
    private void UseItemCallBack(ItemInBagController obj, PointerEventData.InputButton inputButton) {
        UseThisItemCallBack(obj, inputButton);
    }
    /// <summary>
    /// 背包有这个物品添加时调用
    /// </summary>
    ...
    public int AddItem(ItemInfo itemInfo) {
        isNull = false;
        return item.AddNum(itemInfo.num);
    }
    /// <summary>
    /// 添加一个新物品
    /// </summary>
