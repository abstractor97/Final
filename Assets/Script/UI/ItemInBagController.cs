...
public class ItemInBagController : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler,IPointerEnterHandler, IPointerExitHandler,IDropHandler, IPointerClickHandler
{
    ...
    public delegate void ClickItem(ItemInBagController itemInBag, PointerEventData.InputButton inputButton);
    /// <summary>
    /// 使用当前物品的事件
    /// </summary>
    public event ClickItem UseItemCallBack;
    [HideInInspector]
    public Image image;
    [HideInInspector]
    public Text text;
    ...
    {
        UseItem(eventData.button);
    }


    public void UseItem(PointerEventData.InputButton inputButton) {
        if (info.isUse)
        {
            UseItemCallBack(this, inputButton);
        }
