...
public class BagManage : MonoBehaviour, IBeginDragHandler, IDragHandler
{
   ...
   // public GameObject bagItem;
    ...
    public event ItemInBagController.ClickItem UseBagItemCallBack;
    public event ItemInBagController.ClickItem DiscardBagItemCallBack;
    [HideInInspector]
    ...
    protected virtual void UseItemCallBack(ItemInBagController obj, PointerEventData.InputButton inputButton)
    {
        if (extUI!=null)
        {
            if (extUI.GetComponent<BagManage>().BagAddItem(obj.info))
            {
        ...
        UseBagItemCallBack(obj, inputButton);       
    }
    ...
    /// <param name="obj"></param>
    /// <param name="itemInfo"></param>
    private void DiscardItemCallBack(ItemInBagController obj, PointerEventData.InputButton inputButton) {
        DiscardBagItemCallBack(obj, inputButton);
        bagCapacity++;
    }
    /// <summary>
    /// 删除一个物品，无视数量
    ...
    public bool BagUsedItem(string name, int num)
    {
        ...
        ItemInBagController item = GetLatticeItem(name);
        if (item!=null)
        {
            return BagAddItem(new ItemInfo() {
                name = name,
                num=num,
            });
        }
    ...
    /// <summary>
    /// 给背包指定格子添加物品，当该格子被其他物品占用时返回false,这个方法对溢出是不安全的
    /// </summary>
    public virtual bool BagAddItem(int serialNumber, ItemInfo itemInfo)
    {
        ...
        {
            int i = items[serialNumber].AddItem(itemInfo);
            if (i != 0)
            {
                itemInfo.num += i;
                return BagAddItem(itemInfo);
            }
            return true;
        }
        return false;
    ...
    public virtual bool BagAddItem(ItemInfo itemInfo)
    {
        ...
        if (index >= 0)
        {
            int i = items[index].AddItem(itemInfo);
            if (i!=0)
            {
                itemInfo.num += i;
                return BagAddItem(itemInfo);
    ...
    private ItemInfo DataToItem(string[] data) {
        ItemInfo itemInfo = GetItemInfo(data[1]);
    ...
    void Start()
    {
        itemTable = Resources.LoadAll<Item>("Assets/Items");
